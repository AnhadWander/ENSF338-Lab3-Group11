...
for list_length in list_lengths:
    ...
    for _ in range(100): 
        numbers = list(range(list_length, 0, -1)) 
        target = 5
...
# In this code, quicksort has the worst-case time complexity of O(n²) because the array is sorted in descending order.
# This leads to imbalanced partitions in quicksort, which results in poor performance.
# Binary search with quicksort is a little slower compared to the average case, but it's still faster than linear search 
# because binary search operates in O(log n) time, while linear search operates in O(n) time.
# However, as the input size grows, quicksort's inefficiency in the worst case becomes more apparent,
# and binary search with quicksort becomes less favorable for larger arrays.